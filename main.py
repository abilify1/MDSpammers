...
def main():
  ...
  for spam in range(jum):
   ...
   t += 1
   asakita();sunchila();nutriclub();asani();wintershop();datesy();thaifriendly();jumpstart();kinimart();klikwa();bakmikeraton();kopidulukala();kredinesia();pinjamindo();uangpintar();danafix();maucash();omamoriexpress();danacepat();cairin()
#--------------------------------Banner/LOGO------------------------------------------
def logo():
  print """%s
  __  __ ___    ___                                   
...
def danacepat():
 ...
 if json.loads(tu.text)["message"] == 'success':
  sukses("19","sms","danacepat")
 else:
  gagal("19","sms","danacepat")
def cairin():
